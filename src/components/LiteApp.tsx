...
export const LiteApp = React.memo(function LiteApp({
  ...
  useEffect(() => {
    ...
    const loadAjnEpisodes = async () => {
      ...
      try {
        ...
        console.error("Failed to load AJN archive episodes in LiteApp:", err);
      } finally {
  ...
  const resolvedAjnHubSegments = useMemo(() => {
    // Determine the selected day based on real RSS feed.
    // Prioritize July 9th, 2026. Fallback to July 8th, 2026.
    // If neither is present, fallback to the latest date.
    const now = new Date();
    ...
    let targetDay = today;
    let filtered = ajnEpisodes.filter(ep => ep.dateKey === targetDay);
    
    if (filtered.length === 0) {
      targetDay = yesterday;
      filtered = ajnEpisodes.filter(ep => ep.dateKey === targetDay);
    }
    
    const displayDayStr = filtered.length > 0 
      ? (targetDay === today ? "Today" : targetDay === yesterday ? "Yesterday" : targetDay)
      : "Offline";

    const findEpisode = (showKeywords: string[], hourKeywords: string[], fallbackIdx: number) => {
      if (filtered.length === 0) return null;
      const found = filtered.find(ep => {
        const titleLower = ep.title.toLowerCase();
        const matchesShow = showKeywords.some(kw => titleLower.includes(kw));
        const matchesHour = hourKeywords.some(kw => titleLower.includes(kw));
        return matchesShow && matchesHour;
      });
      if (found) return found;
      
      const foundHour = filtered.find(ep => {
        const titleLower = ep.title.toLowerCase();
        return hourKeywords.some(kw => titleLower.includes(kw));
      });
      if (foundHour) return foundHour;

      const foundShow = filtered.find(ep => {
        const titleLower = ep.title.toLowerCase();
        return showKeywords.some(kw => titleLower.includes(kw));
      });
      if (foundShow) return foundShow;

      return filtered[fallbackIdx % filtered.length] || filtered[0];
    };

    const ep1 = findEpisode(["warroom", "war room"], ["hr1", "hour 1", "hour1"], 0);
    const ep2 = findEpisode(["alex", "infowars"], ["hr2", "hour 2", "hour2"], 1);
    const ep3 = findEpisode(["alex", "infowars"], ["hr3", "hour 3", "hour3"], 2);
    const ep4 = findEpisode(["alex", "infowars"], ["hr4", "hour 4", "hour4"], 3);
    const ep5 = findEpisode(["warroom", "war room"], ["hr2", "hour 2", "hour2"], 4);
    const ep6 = findEpisode(["warroom", "war room"], ["hr3", "hour 3", "hour3"], 5);

    const makeSegment = (id: string, defaultTitle: string, defaultUrl: string, ep: any, defaultThumb: string, defaultBroadcaster: string, hourLabel: string) => {
      if (filtered.length === 0) {
        return {
          id,
          title: "No Recent Broadcast Available",
          timestampLabel: "Offline",
          duration: "--:--",
      ...
      const url = ep?.videoUrl || defaultUrl;
      const title = ep ? `${ep.show || defaultBroadcaster} - ${ep.hour || hourLabel}` : defaultTitle;
      return {
        id,
        title: `${title} (${displayDayStr})`,
        timestampLabel: ep?.hour || hourLabel,
        duration: "1:00:00",
        thumbnailUrl: defaultThumb,
        broadcaster: ep?.show?.toUpperCase() || defaultBroadcaster,
        videoUrl: url
      };
    };
    ...
    return [
      makeSegment(
        "seg-warroom-1",
        "War Room Live - Hour 1",
        "https://ajn.archives.pub/hourly-m4v/20260708_Wed_WarRoom-Hr1.m4v",
        ep1,
        "https://archive.org/download/daily-highlights/warroom.png",
        "AJN WAR ROOM",
        "Hour 1"
      ),
      makeSegment(
        "seg-alexjones-2",
        "The Alex Jones Show - Hour 2",
        "https://ajn.archives.pub/hourly-m4v/20260708_Wed_Alex-Hr2.m4v",
        ep2,
        "https://archive.org/download/daily-highlights/gettyimages-1796841914.webp",
        "INFOWARS",
        "Hour 2"
      ),
      makeSegment(
        "seg-live-3",
        "The Alex Jones Show - Hour 3",
        "https://ajn.archives.pub/hourly-m4v/20260708_Wed_Alex-Hr3.m4v",
        ep3,
        "https://archive.org/download/daily-highlights/liberty%20moonlight.png",
        "AJN MAIN FEED",
        "Hour 3"
      ),
      makeSegment(
        "seg-emergency-4",
        "The Alex Jones Show - Hour 4",
        "https://ajn.archives.pub/hourly-m4v/20260708_Wed_Alex-Hr4.m4v",
        ep4,
        "https://archive.org/download/daily-highlights/emegency.png",
        "AJN SYSTEM NETWORK",
        "Hour 4"
      ),
      makeSegment(
        "seg-classic-5",
        "War Room Live - Hour 2",
        "https://ajn.archives.pub/hourly-m4v/20260708_Wed_WarRoom-Hr2.m4v",
        ep5,
        "https://archive.org/download/daily-highlights/old-tv-television-empty-screen-2-cover.jpg",
        "AJN ARCHIVE",
        "Hour 2"
      ),
      makeSegment(
        "seg-highlights-6",
        "War Room Live - Hour 3",
        "https://ajn.archives.pub/hourly-m4v/20260708_Wed_WarRoom-Hr3.m4v",
        ep6,
        "https://archive.org/download/daily-highlights/web%20app1.png",
        "AJN MEDIA DESK",
        "Hour 3"
      )
    ];
            ...
            <ArchiveHub
              ...
              /*
                ...
                },
                {
                  ...
                  videoUrl: "https://archive.org/download/RT_20260630_000000_News/RT_20260630_000000_News.mp4"
                },
                {
                  id: "seg-classic",
